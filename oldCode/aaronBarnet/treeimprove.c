// take in a low stretch spanning tree and try to improve it
#include "graph.h"
...
// Globals and consts
const char* DOC_STRING = "run as ./treeimprove GRAPH.BINIJV TREE.PARRAY IMPROVED_TREE.BINIJV\n";

enum SPLIT_MODES { RANDOM, VOLUME, DIST };   // RANDOM = choose rand edge | VOLUME = edge evenly splitting nodes | DIST = sum of all paths to node is smallest
...
void divideRandom(Tree* tree, TreeNode** nodeTop, TreeNode** nodeBottom)
{
    ...
    while (1)
    {
     randNodeItr = (int)((double)rand()/(double)RAND_MAX * (double)tree->n);
...
Tree* improveTree(Tree* treeIn, myGraph* graph, Tree* masterTree)
{
	...
	for (nbrItr = 0; nbrItr < graph->deg[vert]; nbrItr++)
	{
	    ...
	    {
		double score = curNode->totChildDist + curNode->topDist + graph->wts[vert][nbrItr]
